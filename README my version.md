...
Impossible travel is an authentication anomaly where the same user appears to successfully authenticate from geographically distant locations within a time period that would be unrealistic for normal travel.
...
A human user could not realistically travel between those locations in 30 minutes.

However, impossible travel is an anomaly, not proof of compromise. VPNs, proxies, cloud infrastructure, mobile networks, inaccurate geolocation, and legitimate travel can create similar patterns.

The goal of this lab is therefore to:

Identify an impossible-travel pattern, investigate the surrounding authentication activity, and determine whether the evidence supports a suspicious or benign verdict.
...
A user account shows successful authentication from different geographic locations within a very short period. This may indicate possible account compromise, but legitimate causes such as VPNs or proxy services must also be considered.

In this lab, synthetic authentication data is analyzed in Microsoft Sentinel using KQL to:

- Identify users authenticating from multiple locations.
- Compare authentication timestamps and source IPs.
- Detect unusually short travel intervals.
- Investigate the strongest suspicious activity.
- Assess false positives and evidence gaps.

The primary finding is user1, who authenticates from Hyderabad and New York within 30 minutes, making the activity suspicious and consistent with possible account compromise.

## Lab Objectives

The objectives of this lab are to:

- Understand the concept of impossible-travel authentication activity and why it can indicate possible account compromise.
- Analyze successful authentication events using Microsoft Sentinel and KQL.
- Use synthetic authentication telemetry with KQL datatable() when persistent sign-in telemetry is unavailable.
- Identify users authenticating from multiple geographic locations.
- Compare sequential authentication events using timestamps, source IP addresses, and locations.
- Use KQL functions such as serialize, prev(), dcount(), make_set(), and datetime_diff() for authentication correlation.
- Develop detection logic for authentication events occurring from different locations within a short time period.
- Investigate the strongest impossible-travel candidate and determine whether the activity is suspicious, benign, or inconclusive.
- Evaluate potential false positives, including VPNs, proxies, corporate egress, and inaccurate IP geolocation.
- Distinguish between an authentication anomaly and confirmed account compromise.
- Document confirmed, plausible, and unknown evidence using an evidence-based SOC investigation approach.
- Map the investigation to the relevant MITRE ATT&CK technique.
- Identify additional telemetry required for deeper investigation, such as MFA, Conditional Access, device, sign-in risk, and endpoint data.
