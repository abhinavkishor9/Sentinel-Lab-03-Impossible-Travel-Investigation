...
This investigation examines successful authentication activity to identify possible impossible-travel behavior.
...
The investigation uses synthetic authentication telemetry created through KQL `datatable()` because usable Microsoft Entra `SigninLogs` telemetry was not available in the current Sentinel workspace.

## Investigation Question

The primary question was:

**Did any user successfully authenticate from geographically different locations within a time period that would be difficult or impossible for normal travel?**
...
If equivalent activity were observed in real production telemetry, the next steps would include:

1. Review MFA results.
2. Review Conditional Access decisions.
3. Review device information.
4. Check sign-in risk.
5. Examine authentication methods.
6. Validate IP geolocation.
7. Check VPN or proxy usage.
8. Review endpoint activity.
9. Search for additional authentication events.
10. Review post-authentication activity.
11. Contact the account owner if appropriate.
12. Consider containment if supporting evidence confirms compromise.
...
Impossible travel itself is an anomaly or detection pattern rather than a standalone ATT&CK technique.

## Evidence Classification

### Confirmed

- Successful authentication occurred from Hyderabad.
- Successful authentication occurred from New York.
- The events were 30 minutes apart.
- The source IP changed.
- The application was Microsoft Office.

### Plausible

- Impossible travel.
- Credential misuse.
- VPN or proxy-related location change.
...
The location anomaly increases suspicion, but the analyst should not treat it as proof of compromise without supporting telemetry.
